...
def main():
    ...
    # 创建标准日志器，自动配置控制台输出
    # 使用数值常量20代替logging.INFO，避免循环导入
    logger = create_standard_logger(__name__, level=20)  # 20 = logging.INFO
    logger.info("开始处理采样帧导出任务")
    
    targets_path = os.path.join(args.run_dir, 'analysis_targets.json')
    ...
    ms.to_deepmd_npy(args.output_dir)
    logger.info(f"已导出deepmd npy至 {args.output_dir}")
    
    # 拆分
    if args.split_ratio:
        split_and_save(ms, args.output_dir, args.split_ratio, logger)
