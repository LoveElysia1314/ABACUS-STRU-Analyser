...
def analyse_sampling_compare(result_dir=None):
    ...
    if rows:
        _save_comparison_results(rows, result_dir)
...
def _analyze_single_system(file_path):
    ...
    try:
        ...
        logger.error(f"分析系统 {file_path} 时出错: {e}")
        return None
...
def _save_comparison_results(rows, result_dir):
    """保存比较结果"""
    single_dir = os.path.join(result_dir, 'single_analysis_results')
...
def _create_summary_table(rows, result_dir):
    """创建汇总表格"""
    if not rows:
